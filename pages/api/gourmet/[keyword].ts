...
const handler = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  ...
  const keywordString: string | undefined = Array.isArray(keyword) ? keyword.join(' ') : keyword

  const API_URL =
    keyword === '' || typeof keyword === 'undefined' || keyword === null || typeof keywordString === 'undefined'
      ? API_URL_ROOT
